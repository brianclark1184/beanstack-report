...
for(const size of [16,48]) await sharp(svg).resize(size,size).png().toFile(`extension/icons/${size}.png`);
// Web Store icon artwork is 96x96 within a transparent 128x128 canvas.
await sharp(svg).resize(96,96).extend({top:16,bottom:16,left:16,right:16,background:{r:0,g:0,b:0,alpha:0}}).png().toFile('extension/icons/128.png');
